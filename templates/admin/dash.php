...
<script>async function git(a){const o=document.getElementById('go');o.style.display='block';o.textContent='⏳…';try{const r=await fetch('/admin.php?act=git',{method:'POST',body:JSON.stringify({action:a})});o.textContent=JSON.stringify(await r.json(),null,2)}catch(e){o.textContent=e.message}}</script>
